...
def xtest_clear_req_channel_master_uri_override(temp_salt_minion, temp_salt_master):
    """
...
async def test_req_chan_decode_data_dict_entry_v2(minion_opts, master_opts, pki_dir):
    ...
    client.auth.session_crypticle.dumps = auth.crypticle.dumps
    client.auth.session_crypticle.loads = auth.crypticle.loads
...
async def test_pub_client_init(minion_opts, io_loop):
...
async def test_req_chan_bad_payload_to_decode(
    minion_opts, master_opts, pki_dir, io_loop, caplog
):
    minion_opts.update(
        {
            "master_uri": "tcp://127.0.0.1:4506",
