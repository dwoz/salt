...
def test_script(salt_ssh_cli_parameterized):
    args = "saltines crackers biscuits=yes"
    script = "salt://script.py"
    ret = salt_ssh_cli_parameterized.run("cmd.script", script, args)
...
def test_script_query_string(salt_ssh_cli_parameterized):
    args = "saltines crackers biscuits=yes"
...
def test_script_cwd(salt_ssh_cli_parameterized, tmp_path):
    args = "saltines crackers biscuits=yes"
    script = "salt://script.py"
    # can't pass cwd as kwarg
    ret = salt_ssh_cli_parameterized.run("cmd.script", script, args, tmp_path)
    assert ret.returncode == 0
    assert isinstance(ret.data, dict)
    assert ret.data
    assert ret.data["stdout"] == args
...
def test_script_cwd_with_space(salt_ssh_cli_parameterized, tmp_path):
    tmp_cwd = tmp_path / "test 2"
    tmp_cwd.mkdir()
    args = "saltines crackers biscuits=yes"
    script = "salt://script.py"
    ret = salt_ssh_cli_parameterized.run("cmd.script", script, args, cwd=tmp_cwd)
    assert ret.returncode == 0
    assert isinstance(ret.data, dict)
    assert ret.data
    assert ret.data["stdout"] == args
...
def test_script_nonexistent(salt_ssh_cli_parameterized, template):
    script = "salt://non/ex/is/tent.sh"
    ret = salt_ssh_cli_parameterized.run("cmd.script", script, "", template=template)
...
def test_script_template(salt_ssh_cli_parameterized, script_templated, pillar):
...
def test_script_retcode(salt_ssh_cli_parameterized):
    script = "salt://script.py"
    ret = salt_ssh_cli_parameterized.run("cmd.script_retcode", script)
    assert ret.returncode == 0
    assert ret.data == 0
