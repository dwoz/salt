...
def test_dateutils_strftime(salt_master, salt_ssh_cli_parameterized):
    """
    test jinja filter datautils.strftime (parameterized for both thin and relenv)
    """
    sls_contents = """
    ...
    with salt_master.state_tree.base.temp_file("dateutils.sls", sls_contents):
        ret = salt_ssh_cli_parameterized.run("state.sls", "dateutils")
