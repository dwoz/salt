...
class MinionBase(object):
    ...
    def gen_modules(self, initial_load=False, context=None):
        ...
        if initial_load:
            ...
            pillar = pillar_obj.compile_pillar()
            self.opts["pillar"] = pillar
            ...
            while True:
                ...
                try:
                    ...
                    if hasattr(self, "job_q"):
                        self.job_q.put(
                            {"kind": "master_uri", "data": self.opts["master_uri"]}
                        )
                        self.is_ready.set()
                    raise salt.ext.tornado.gen.Return((opts["master"], pub_channel))
                except SaltClientError:
        ...
        while True:
            ...
            if payload["kind"] == "master_uri":
                thread = threading.Thread(
                    target=self.spawn_job, args=(minion_instance, payload, True)
                )
                thread.start()
        ...
        try:
            ...
            if payload["kind"] == "master_uri":
                ...
                minion_instance.beacons = salt.beacons.Beacon(
                    minion_instance.opts, minion_instance.functions
                )
...
class Minion(MinionBase):
    ...
    def __init__(
        ...
        if top and not hasattr(self, "job_q"):
            ...
            self.job_spawner_proc = JobSpawner(self.__class__, self.job_q, self.opts,)
    ...
    def beacons_refresh(self):
        ...
        if hasattr(self, "job_q"):
            self.job_q.put({"kind": "refresh_beacons"})
    ...
    def _handle_payload(self, payload):
        self.job_q.put(
            {"kind": "payload", "data": payload}, block=False,
        )
